...
class PreTrainedEmbeddings(object):
    ...
    Example:
        ...
        >>> # Get the n closest words to a given vector
        >>> cat_word_vector = embeddings_wrapper.word_vectors[
        ...     embeddings_wrapper.word_to_index["cat"]
        ... ]
        >>> closest_words = embeddings_wrapper.get_closest_to_vector(
        ...     vector=cat_word_vector, n=5
        ... )
        >>> print(closest_words)
        ['cat', 'dog', 'rabbit', 'cats', 'pet']
    """
    ...
    def __init__(
        ...
        self.index: AnnoyIndex = AnnoyIndex(
            f=len(self.word_vectors[0]), metric="euclidean"
        )

        tqdm_bar = tqdm(
            iterable=self.word_to_index.items(),
            position=0,
            leave=True,
            desc="Building Index ...",
            colour="green",
        ...
        for _, i in tqdm_bar:
            self.index.add_item(i=i, vector=self.word_vectors[i].cpu().numpy())
    ...
    def get_closest_to_vector(
        self, vector: numpy.ndarray | torch.Tensor, n: int = 3
    ) -> List[str]:
        """Given a vector, returns its `n` nearest neighbors.

        Args:
            vector (numpy.ndarray | torch.Tensor): Should match the
                size of the vectors in the `Annoy` index.
            n (int): Number of neighbors to return.

        Returns:
            List[str]: `n` words that are nearest to the given vector.
                The words are not ordered by distance.
        """
        if isinstance(vector, torch.Tensor):
            vector = vector.cpu().numpy()

        nn_indices = self.index.get_nns_by_vector(vector=vector, n=n)  # type: ignore
